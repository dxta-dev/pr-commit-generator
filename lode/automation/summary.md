...
The runner lists automation PRs with `gh pr list` and filters them by branch prefix or label using `jq`. Merge targets skip rebasing and go straight to squash auto-merge (`gh pr merge --squash --auto`) so non-mergeable PRs queue instead of hard failing. `AUTOMATION_FILE_PATH` provides the directory prefix for heartbeat files.
