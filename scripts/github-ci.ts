...
const heartbeatPath = process.env.AUTOMATION_FILE_PATH ?? "automation/heartbeat.txt";
...
const appendHeartbeat = (branchName: string) => {
  ...
  appendFileSync(heartbeatPath, `${new Date().toISOString()} ${branchName}\n`, "utf8");
};
...
const isAutomationPr = (pr: { head: { ref: string }; labels?: Array<{ name?: string } | string> }) => {
...
const updatePullBranches = async (pulls: Array<{ number: number; head: { ref: string } }>) => {
  for (const pr of pulls) {
...
const main = async () => {
  ...
  const closeTargets = sample(openPulls, ratioCount(openPulls.length, closeRatio));
  const closeSet = new Set(closeTargets.map((pr) => pr.number));
  await closePullRequests(closeTargets);

  const remainingAfterClose = openPulls.filter((pr) => !closeSet.has(pr.number));
  ...
  const remainingAfterUpdate = remainingAfterClose.filter((pr) => !updateSet.has(pr.number));
